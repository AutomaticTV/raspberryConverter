...
func SetNetworkConfig(config Network) error {
  configFile, err := getNetConfigFile()
  if err != nil { return err }
  if isDHCP(configFile) && config.Mode == "DHCP" { return nil }
  withoutStatic := removeStaticConfig(configFile)
  if config.Mode == "DHCP" {
    return setNetConfigFile(withoutStatic)
  }
  ...
  err = setNetConfigFile(withoutStatic + buff.String())
  if err != nil {
    return err
  }
  return resetNetwork()
...
func GetNetworkConfig() (interface{}, error) {
  ...
  configFile, err := getNetConfigFile()
  if err != nil { return networkError(err) }
  if isDHCP(configFile) { Mode = "DHCP"  }
  IP := ""
...
func getNetConfigFile() (string, error) {
  tmp, err := exec.Command("cat", "/etc/dhcpcd.conf").CombinedOutput()
  return string(tmp), err
}

func setNetConfigFile(content string) error {
